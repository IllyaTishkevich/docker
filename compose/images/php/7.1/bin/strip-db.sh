#!/usr/bin/env bash
echo "starting stripping magento's 2 database"
PASSWORD="magento"
HOST="db"
PORT="3306"
USER="root"
...
DEVELOPMENT_TABLES=$ADMIN_TABLES" "$TRADE_TABLES" "$STRIPPED_TABLES" "$SEARCH_TABLES" "$TFA_TABLES
TRUNCATED_TABLES=''
for tablename in $DEVELOPMENT_TABLES; do
    TRUNCATED_TABLES+="CREATE TABLE IF NOT EXISTS ${DATABASE}.\`${tablename}\` (m INT); TRUNCATE TABLE ${DATABASE}.\`${tablename}\`; "
done
if  [ -x "$(command -v mysql)" ]; then
mysql --port=${PORT} --host=${HOST} -u ${USER} -p${PASSWORD} ${DATABASE} <<EOF
USE $DATABASE;
...
EOF
   ...
   echo "mysql utility is not installed"
fi
